...
async def exchange(request: Request):
    ...
    if resp.status_code != 200:
        ...
        # Keycloak may not have token-exchange enabled — fall back to signed local OBO
        # (same behavior as before, but logged clearly as fallback)
        log.warning("Falling back to local OBO token (Keycloak token-exchange not configured)")
        return _local_obo_fallback(subject_token, actor_token, scope)
...
def _local_obo_fallback(subject_token: str, actor_token: str, scope: str):
    """
    Fallback: mint a locally-signed OBO JWT when Keycloak token-exchange
    is not configured. Structure is identical to what Keycloak would return.
    Used only when Keycloak rejects the exchange (setup incomplete).
    """
...
async def refresh_token(request: Request):
    ...
    if rt_claims.get("_fallback") or rt_claims.get("token_type") == "refresh":
        ...
        def b64url(d): return base64.urlsafe_b64encode(d).decode().rstrip("=")
        h = b64url(json.dumps({"alg":"HS256","typ":"JWT"}).encode())
        p = b64url(json.dumps({"iss":KC_ISSUER,"sub":user_sub,"act":{"sub":agent_sub},
            "scope":scope,"jti":uuid.uuid4().hex,"iat":now,"exp":now+3600,
            "token_type":"Bearer","_fallback":True}).encode())
        sig = b64url(_hmac.new(SECRET, f"{h}.{p}".encode(), hashlib.sha256).digest())
        rp = b64url(json.dumps({"iss":KC_ISSUER,"sub":user_sub,"act":{"sub":agent_sub},
            "jti":uuid.uuid4().hex,"iat":now,"exp":now+86400,"token_type":"refresh"}).encode())
        rsig = b64url(_hmac.new(SECRET, f"{h}.{rp}".encode(), hashlib.sha256).digest())
        log.info(f"[REFRESH/FALLBACK] sub={user_sub} act={agent_sub}")
        return JSONResponse({"access_token":f"{h}.{p}.{sig}","refresh_token":f"{h}.{rp}.{rsig}",
    ...
    resp = _http.post(KC_TOKEN_URL, data=kc_form,
                      headers={"Accept": "application/json",
                               "Content-Type": "application/x-www-form-urlencoded"})
    if resp.status_code != 200:
...
async def authz(request: Request):
    ...
    if resp.status_code != 200:
        # fallback
        fallback_resp = _local_obo_fallback(user_token, actor_token, DEFAULT_SCOPE)
        obo_data = json.loads(fallback_resp.body)
    else:
        obo_data = resp.json()
...
def healthz():
    kc_ok = False
    try:
        r = _http.get(f"{KC_ISSUER}/.well-known/openid-configuration", timeout=3)
        kc_ok = r.status_code == 200
    except Exception:
        pass
    return {"status": "ok", "service": "obo-exchange", "keycloak_reachable": kc_ok}
